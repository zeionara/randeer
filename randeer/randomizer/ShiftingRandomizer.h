...
struct ShiftingRandomizer: Randomizer<T> {
    ...
    T nextInIntervalExcluding() {
        ...
        if (number > context->greatestExcluded - context->nExcluded) // > greatest not excluded before the first excluded in case excluded numbers are consequitive numbers like 1, 2, 3, 4, ...
